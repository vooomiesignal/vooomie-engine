...
# AI_PROVIDER: "grok", "gemini", "anthropic", or "auto" (tries best available)
...
{{
  "headline": "punchy headline under 14 words, Nigerian English tone",
  "subheadline": "one sentence explainer under 20 words",
  "hook": "opening sentence that makes someone stop scrolling",
  "body_paragraph_1": "what happened — 2-3 sentences, conversational, factual",
  "body_paragraph_2": "why it matters / reactions — 2-3 sentences",
  "body_paragraph_3": "what happens next — 1-2 sentences, end with question for comments",
  "tiktok_caption": "under 150 chars, viral hook + 3 hashtags including #VooomieNG",
  "instagram_caption": "engaging 100-200 chars + 8 hashtags including #VooomieNG #NaijaNews",
  "whatsapp_blast": "broadcast message max 200 chars, ends with 👉 [LINK]",
  "telegram_post": "short post with **bold** headline, 2 sentences, [LINK]",
  "seo_title": "under 60 chars keyword-rich title",
  "meta_description": "under 155 chars with main keyword",
  "focus_keyword": "main SEO keyword",
  "tags": ["tag1","tag2","tag3","tag4","tag5"],
  "image_search_query": "3-5 word Unsplash search query for thumbnail"
}}"""

def ai_rewrite_gemini(story):
    """Rewrite using Google Gemini API (free tier)."""
    ...
    try:
        ...
        if res.status_code != 200:
            log.warning(f"  Gemini error {res.status_code}: {res.text[:200]}")
            return None
        ...
        log.info(f"  ✦ Gemini rewrote: {result.get('headline','')[:60]}")
        return result
    except Exception as e:
        log.warning(f"  Gemini rewrite failed: {e}")
        return None


def ai_rewrite_grok(story):
    """Rewrite using xAI Grok API (free tier)."""
    if not GROK_API_KEY:
        return None
    try:
        prompt = REWRITE_PROMPT.format(
        ...
        if res.status_code != 200:
            log.warning(f"  Grok error {res.status_code}: {res.text[:200]}")
            return None
        text = res.json()["choices"][0]["message"]["content"]
        import re as _re
        text = _re.sub(r"```json|```","", text).strip()
        result = json.loads(text)
        log.info(f"  ✦ Grok rewrote: {result.get('headline','')[:60]}")
        return result
    except Exception as e:
        log.warning(f"  Grok rewrite failed: {e}")
        return None


def ai_rewrite_anthropic(story):
    ...
    try:
        ...
        text = _re.sub(r"```json|```","", text).strip()
        result = json.loads(text)
        log.info(f"  ✦ Claude rewrote: {result.get('headline','')[:60]}")
        return result
    except Exception as e:
        log.warning(f"  Claude rewrite failed: {e}")
        return None


def ai_rewrite(story):
    """
    Smart AI rewrite — tries providers in order of availability.
    Priority: Grok → Gemini → Anthropic → Fallback
    Configurable via AI_PROVIDER env var.
    """
    providers = {
        "grok":      ai_rewrite_grok,
        "gemini":    ai_rewrite_gemini,
        "anthropic": ai_rewrite_anthropic,
    }

    if AI_PROVIDER == "auto":
        # Try in order: Grok first (fastest), then Gemini, then Claude
        order = ["grok", "gemini", "anthropic"]
    elif AI_PROVIDER in providers:
        order = [AI_PROVIDER]
    else:
        order = ["grok", "gemini", "anthropic"]
    ...
    for provider in order:
        ...
        if provider == "gemini" and not GEMINI_API_KEY:
            continue
        if provider == "anthropic" and not client:
            continue

        log.info(f"  Trying AI rewrite with {provider.upper()}...")
        result = providers[provider](story)
        if result:
            log.info(f"  ✦ AI rewrite SUCCESS via {provider.upper()}")
            return result

    # All AI providers failed or unavailable
    log.warning("  No AI available — using smart fallback content")
