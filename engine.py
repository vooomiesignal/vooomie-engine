...
CATEGORIES = {
    "sports":     ["super eagles", "afcon", "premier league", "champions league", "nba", "nfl", "transfer", "goal", "match", "chelsea", "arsenal", "psg", "fifa", "uefa", "world cup", "la liga", "bundesliga", "football", "soccer", "basketball", "tennis", "athletics", "olympic", "sports", "tournament", "league", "cup final", "arteta", "rice", "mbappe", "ronaldo", "messi"],
    "politics":   ["senate", "president", "governor", "minister", "election", "aso rock", "nass", "tinubu", "atiku", "peter obi", "APC", "PDP", "labour party", "parliament", "congress", "government", "policy", "vote", "democracy", "inauguration", "impeach"],
    "celebrity":  ["wizkid", "burna boy", "davido", "tiwa savage", "olamide", "adekunle gold", "tems", "rema", "ayra starr", "album", "tour", "grammy", "bet awards", "afrobeats", "musician", "singer", "actor", "actress", "nollywood", "billboard"],
    "crime":      ["arrested", "efcc", "police", "court", "jail", "fraud", "robbery", "kidnap", "IPOB", "bandits", "terrorism", "murder", "killed", "attack", "shooting", "arson", "stolen", "suspect"],
    "business":   ["naira", "CBN", "economy", "investment", "profit", "stock", "oil", "revenue", "company", "billion", "million", "market", "trade", "export", "import", "gdp", "inflation", "interest rate", "bank"],
    "tech":       ["startup", "artificial intelligence", "AI model", "app launch", "software", "funding round", "fintech", "crypto", "blockchain", "silicon", "developer", "programming", "cybersecurity", "data breach", "elon musk tech", "spacex", "tesla tech"],
    "funny":      ["hilarious", "lol", "comedy", "joke", "funny", "prank", "meme", "skit", "viral video", "goes viral", "twitter reacts", "reactions", "social media laughs"],
    "lifestyle":  ["fashion", "style", "beauty", "food", "restaurant", "travel", "luxury", "wedding", "health", "fitness", "relationship", "marriage", "family"],
}
...
def post_to_twitter(content, post_url, image_url=None):
    ...
    if not all([TWITTER_API_KEY, TWITTER_API_SECRET, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_SECRET]):
        log.info("  Twitter credentials not set — skipping")
        return False
    try:
        import hmac
        import hashlib
        ...
        # Build tweet text
        tweet = content.get("tiktok_caption", content.get("headline", ""))
        # Twitter limit: 280 chars. Leave room for URL
        max_text = 255
        if len(tweet) > max_text:
            tweet = tweet[:max_text-3] + "..."
        tweet_text = tweet + "\n\n" + post_url
        ...
        all_params = {**oauth_params, "text": tweet_text}
        sig = oauth_signature("POST", "https://api.twitter.com/2/tweets",
                               all_params, TWITTER_API_SECRET, TWITTER_ACCESS_SECRET)
        ...
        if res.status_code in [200, 201]:
            ...
            log.error(f"  Twitter error {res.status_code}: {res.text[:200]}")
            return False
    except Exception as e:
...
def post_to_facebook(content, post_url, image_url=None):
    ...
    try:
        ...
        if image_url:
            # Post with image (photo post — more engagement)
            res = requests.post(
...
def publish_story(story):
    ...
    if TWITTER_API_KEY:
        ...
        log.info("  Twitter/X: add credentials to Railway to enable")
    ...
    if FACEBOOK_PAGE_ID:
        ...
        log.info("  Facebook: add credentials to Railway to enable")
